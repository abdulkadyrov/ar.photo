...
The production GitHub Actions workflow keeps one hosted worker polling for up to five and a half hours. Scheduled runs form a handoff so new uploads do not depend on GitHub's delayed cron delivery; it can also be started manually. It uses repository secrets named `PROCESSING_SUPABASE_URL` and `PROCESSING_SUPABASE_SERVICE_ROLE_KEY`; the service credential is passed only to the isolated worker container.
