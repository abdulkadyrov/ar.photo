...
Optional controls are `PROCESSING_CONCURRENCY` (1–4, default 1), `PROCESSING_POLL_INTERVAL_MS` (250–60000, default 2000), `PROCESSING_RUN_ONCE=1` for a single polling cycle, `PROCESSING_IDLE_POLLS_BEFORE_EXIT` (0–60, default 0) for a queue-draining batch that exits cleanly after the configured number of empty polls, and `PROCESSING_MAX_RUNTIME_MS` (up to six hours, default 0/unlimited) for a graceful hosted-runner handoff.
...
The test-stage GitHub Actions workflow starts every five minutes, drains the current queue with up to four concurrent jobs, waits for three empty polls, and exits. Short repeated runs are intentionally used instead of treating a hosted CI runner as a permanent server; a delayed or unavailable runner gets another chance on the next schedule. The workflow can also be started manually. It uses repository secrets named `PROCESSING_SUPABASE_URL` and `PROCESSING_SUPABASE_SERVICE_ROLE_KEY`; the service credential is passed only to the isolated worker container.
